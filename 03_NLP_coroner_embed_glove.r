...
###Load libraries
library(pacman)
p_load(tidyverse, purrr, forcats, visdat, flextable, gtsummary, colorspace, tidymodels, tictoc) #Basic
p_load(textrecipes, texdata, tidytext) #Text processing helpers
p_load(parsnip, discrim, naivebayes, ranger, xgboost, kknn, keras, workflowsets, themis, stacks) #Models
doMC::registerDoMC(cores = parallel::detectCores(logical = FALSE))
set.seed(100)
...
wflw_set.list <- map(rec.list, ~ workflow_set(
      preproc = list(.x), 
      models = list(Logistic.regression = log_spec, Naive.Bayes = nb_spec, 
...
tic()
grid_results.list <- map(wflw_set.list, ~
   .x %>%
   workflow_map(
      seed = 100,
      resamples = cv_folds,
...
saveRDS(grid_results.list, paste0("Output/grid_results", as.character(Sys.Date()),"embed_glove.rds"))
...
#Create PR cuve by model/outcome
...
#Fit on train data
tic()
test_results <- map(1:length(outcomes), ~ {  
   grid_results.list[[.x]] %>% 
...
#Save/Load to local harddrive
saveRDS(test_results, paste0("Output/test_results", as.character(Sys.Date()),"embed_glove.rds"))
...
openxlsx::write.xlsx(error_analysis_text, paste0("Error/error_analysis_df_", Sys.Date(), "embed_glove.xlsx"))
...
#Save/Load to local harddrive
saveRDS(bootstrap_results, paste0("Output/bootstrap_results", as.character(Sys.Date()),"embed_glove.rds"))
